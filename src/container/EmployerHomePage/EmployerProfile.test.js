...
describe('EmployerProfile Component', () => {
  beforeEach(() => {
    sessionStorage.setItem('user', 'testToken');
  });
  ...
  test('should update profile successfully with valid data', async () => {
    updateCompanyProfile.mockResolvedValue({ _id: '12345' });

    render(<EmployerProfile />);

    fireEvent.change(screen.getByLabelText('Company Name'), { target: { value: 'TechCorp' } });
    fireEvent.change(screen.getByLabelText('Industry'), { target: { value: 'Software' } });
    fireEvent.change(screen.getByLabelText('Description'), { target: { value: 'A leading software company.' } });
    fireEvent.change(screen.getByLabelText('Street'), { target: { value: '123 Tech Street' } });
    fireEvent.change(screen.getByLabelText('City'), { target: { value: 'Tech City' } });
    fireEvent.change(screen.getByLabelText('Country'), { target: { value: 'Tech Country' } });
    fireEvent.change(screen.getByLabelText('Postal'), { target: { value: '123456' } });
